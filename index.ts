...
// Triggered automatically by a Postgres trigger whenever bookings.status changes.
// Sends an email to the customer via Resend.
...
serve(async (req) => {
  try {
    ...
    // Expecting the Postgres trigger to send { record: {...booking row...}, old_record: {...} }
